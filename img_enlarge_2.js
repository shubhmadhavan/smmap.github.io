$(document).ready(function () {
    ...
    // Handle click on images inside `.info-text-4` elements to show overlay
    $(document).on('click', '.info-text-4 img', function () {
        const $parent = $(this).closest('.info-text-4'); // Get parent .info-text-4
    ...
    $(document).keydown(function (e) {
        if (e.key === 'e' || e.key === 'E') {
            if (overlay) {
                ...
                const firstImage = $('.info-text-4 img').first(); // Get the first image
